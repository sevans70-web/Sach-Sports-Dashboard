...
PROP_CATALOG = {
    ...
    "Tackles + Assists": {"builder": build_tackles_assists_top25, "projection": "tackles_projection", "unit": "tackles", "icon": "🤝"},
}
...
def _inject_nfl_css() -> None:
    st.markdown(
        ...
        .nfl-hero{margin:0 0 6px;padding:10px 12px;border-radius:15px;background:linear-gradient(105deg,rgba(255,204,51,.28) 0%,rgba(4,5,4,.98) 44%,rgba(25,217,120,.28) 100%);border:2px solid rgba(255,204,51,.88);box-shadow:inset 0 0 24px rgba(25,217,120,.08),0 0 0 1px rgba(25,217,120,.18);overflow:hidden}
        ...
        .nfl-hero-subtitle{margin:6px 0 0!important;color:#f0f0f0!important;font-size:.95rem!important;line-height:1.45!important;max-width:900px}

        div[class*="st-key-nfl_games_entry"]{margin-bottom:-.20rem!important}
        div[class*="st-key-nfl_games_entry"] button{width:100%!important;min-height:78px!important;padding:10px 13px!important;margin:2px 0 7px!important;text-align:left!important;justify-content:flex-start!important;border:1.5px solid rgba(214,179,92,.68)!important;border-left:5px solid #19d978!important;border-radius:13px!important;background:linear-gradient(112deg,rgba(246,200,76,.12) 0%,#0d0f10 36%,#0b0d0e 68%,rgba(25,217,120,.10) 100%)!important;color:#fff!important;font-weight:900!important;white-space:pre-line!important;line-height:1.28!important}
        ...
        div[class*="st-key-nfl_games_entry"] button [data-testid="stMarkdownContainer"]{width:100%!important;text-align:left!important;margin:0!important;padding:0!important}
        div[class*="st-key-nfl_games_entry"] button p{width:100%!important;margin:0!important;padding:0!important;text-align:left!important;font-size:.84rem!important;line-height:1.32!important;white-space:pre-line!important}

        .nfl-snapshot-heading{margin:8px 0 6px;color:#fff;font-size:1.08rem;font-weight:950;white-space:nowrap}
        .nfl-snapshot-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin-bottom:10px}
        ...
        .nfl-rankings-heading{margin:14px 0 7px}.nfl-rankings-heading strong{display:block;color:#fff;font-size:1.28rem;font-weight:950}.nfl-rankings-heading span{display:block;color:#c4c7cc;font-size:.80rem;line-height:1.35;margin-top:4px}
        ...
        div[class*="st-key-nfl_rank_wrap_"] button{width:calc(100% - 20px)!important;margin:5px 10px 4px!important;background:#080909!important;color:#fff!important;border:2px solid rgba(25,217,120,.72)!important;border-radius:9px!important;min-height:38px!important;font-weight:850!important;justify-content:center!important;text-align:center!important}
        div[class*="st-key-nfl_rank_wrap_"] button:hover{border-color:#ffcc33!important;color:#ffcc33!important}
        .nfl-intel-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:5px;padding:2px 10px 8px}
        .nfl-intel-metric{background:#111315;border:1px solid #30343a;border-radius:8px;padding:7px 6px;min-width:0}.nfl-intel-metric:nth-child(odd){border-left:3px solid #19d978;border-bottom:2px solid rgba(25,217,120,.60)}.nfl-intel-metric:nth-child(even){border-left:3px solid #d6b35c;border-bottom:2px solid rgba(214,179,92,.68)}.nfl-intel-metric span{display:block;color:#92979e;font-size:.57rem}.nfl-intel-metric strong{display:block;color:#fff;font-size:.82rem;margin-top:3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
        .nfl-why{margin:0 10px 9px;padding:9px 10px;border:1px solid rgba(214,179,92,.48);border-radius:9px;background:#101112;color:#d9dbde;font-size:.72rem;line-height:1.4}.nfl-why b{display:block;color:#f6c84c;margin-bottom:3px}
...
def _detail_metrics(row: pd.Series, prop: str) -> list[tuple[str, str]]:
    ...
    raw_mode = row.get("ranking_mode")
    mode = "Baseline" if raw_mode is None or pd.isna(raw_mode) else str(raw_mode)
    if mode.strip().lower() == "foundation":
        mode = "Baseline"
...
def _why_engine(row: pd.Series, prop: str) -> str:
    parts = []
    raw_matchup_label = row.get("passing_matchup_label")
    matchup_label = "" if raw_matchup_label is None or pd.isna(raw_matchup_label) else str(raw_matchup_label).strip()
    ...
    line = row.get("consensus_line")
    raw_side = row.get("model_side")
    side = "" if raw_side is None or pd.isna(raw_side) else str(raw_side).strip()
...
def _lineup_status_html(row: pd.Series) -> str:
    ...
    if confirmed is not None and not pd.isna(confirmed) and bool(confirmed):
        return '<span class="nfl-lineup-status nfl-lineup-confirmed">✓ Confirmed</span>'
    if "confirmed" in status and "unconfirmed" not in status:
        return '<span class="nfl-lineup-status nfl-lineup-confirmed">✓ Confirmed</span>'
...
def _render_rank_header(row: pd.Series, prop: str) -> None:
    ...
    lineup_status = _lineup_status_html(row)
    line = row.get("consensus_line")
    if line is None or pd.isna(line):
        line = row.get("prop_line")
    if line is not None and not pd.isna(line):
        market_mode = f"Market line {float(line):.1f}"
    else:
        _, recent = _first_numeric(row, [k for k in row.index if str(k).startswith("last_5_")])
        market_mode = "Baseline projection" if recent is None else f"Baseline projection · L5 {recent:.1f}"
    _render_html(
        f"""
...
def _render_player_intelligence(row: pd.Series, prop: str, key_prefix: str) -> None:
    ...
    _render_html(f'<div class="nfl-why"><b>Why This Player Ranks Here</b>{escape(_why_engine(row, prop))}</div>')
...
def show() -> None:
    ...
    _render_html(
        ...
        <div class="nfl-snapshot-grid">
          ...
          <div class="nfl-snapshot-card"><span>LINEUPS</span><strong>—</strong></div>
