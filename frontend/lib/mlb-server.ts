...
function statusGroup(abstractState: string, detailed: string) {
  ...
  if (a === "live" && !/(delay|postpon|suspend|warmup)/.test(d)) return "live" as const;
  if (a === "preview" || /(delay|postpon|suspend|warmup|scheduled|pre-game)/.test(d)) return "preview" as const;
  return "other" as const;
}

export async function getSchedule(date?: string): Promise<{ games: MlbGame[]; fetchedAt: string; date: string }> {
  const torontoDate = date || new Intl.DateTimeFormat("en-CA", { timeZone: TORONTO, year: "numeric", month: "2-digit", day: "2-digit" }).format(new Date());
  const url = new URL(MLB_SCHEDULE);
  url.searchParams.set("sportId", "1");
  url.searchParams.set("date", torontoDate);
  ...
  const games: MlbGame[] = rawGames.map((game: any) => {
    ...
    const startTime = gameDate
      ? new Intl.DateTimeFormat("en-US", { timeZone: TORONTO, hour: "numeric", minute: "2-digit", timeZoneName: "short" }).format(new Date(gameDate))
      : "Time TBA";
    ...
    const side = (name: "away" | "home") => ({
      ...
      probablePitcher: String(safe(game, ["teams", name, "probablePitcher", "fullName"], "Pitcher TBA")),
    });
    ...
    return {
      ...
      venue: String(safe(game, ["venue", "name"], "Venue TBA")),
      away: side("away"), home: side("home"),
    };
  });

  return { games, fetchedAt: new Date().toISOString(), date: torontoDate };
}
...
async function supabaseRows(path: string) {
  ...
  if (!url || !key) return { rows: [] as any[], connected: false, error: "SUPABASE_URL / SUPABASE_KEY are not available to the Next.js Railway service." };
  try {
...
export async function getSourceSnapshot(sourceName: string) {
  ...
  const result = await supabaseRows(`source_snapshots?select=id,source_name,game_date,payload,created_at&source_name=eq.${encoded}&order=created_at.desc&limit=1`);
...
export async function getRankings() {
  ...
  const batterPayload = batters.payload || {};
  const pitcherPayload = pitchers.payload || {};
  ...
  for (const key of ["home_runs","hits","total_bases","runs","rbis","walks","stolen_bases","hits_runs_rbis"]) {
    const value = batterPayload?.[key];
    batter[key] = Array.isArray(value) ? value : Array.isArray(value?.rankings) ? value.rankings : [];
  }
  const sourcePitcher = pitcherPayload?.rankings || pitcherPayload;
  const pitcher: Record<string, RankingRow[]> = {};
  for (const key of ["strikeouts","outs_recorded","hits_allowed","walks_allowed","earned_runs"]) {
    const value = sourcePitcher?.[key];
    pitcher[key] = Array.isArray(value) ? value : Array.isArray(value?.rankings) ? value.rankings : [];
  }
  return {
    batter, pitcher,
    connected: batters.connected || pitchers.connected,
    batterConnected: batters.connected,
    pitcherConnected: pitchers.connected,
...
export async function getPerformance() {
  ...
  return {
    ...
    batter: batter.payload || {}, pitcher: pitcher.payload || {}, emerging: emerging.payload || {},
...
export async function getGameFeed(gamePk: string) {
  ...
  const lineup = (side: "away" | "home") => {
    ...
    return order.map((id, i) => {
      ...
      return {
        playerId: id,
        name: boxPlayer?.person?.fullName || detail?.fullName || `MLB Player ${id}`,
        position: boxPlayer?.position?.abbreviation || detail?.primaryPosition?.abbreviation || "",
        battingOrder: i + 1,
      };
    });
  };
  ...
  const side = (name: "away" | "home") => ({
    id: Number(teams?.[name]?.id || 0),
    name: String(teams?.[name]?.name || name),
    pitcher: String(probable?.[name]?.fullName || "Pitcher TBA"),
    score: safe(feed, ["liveData", "linescore", "teams", name, "runs"], null),
    lineup: lineup(name),
  });

  return {
    gamePk: Number(gamePk), status, statusGroup: group,
    isDelayed: /(delay|postpon|suspend)/i.test(status),
    venue: String(safe(feed, ["gameData", "venue", "name"], "Venue TBA")),
    away: side("away"), home: side("home"),
...
export async function getPlayer(playerId: string) {
  ...
  const statsRes = await fetch(`https://statsapi.mlb.com/api/v1/people/${encodeURIComponent(playerId)}/stats?stats=gameLog,season&group=hitting&season=2026`, { next: { revalidate: 300 } });
