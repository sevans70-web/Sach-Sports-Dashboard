...
function marketMatch(v:any,m:CfbMarketKey){
  const x=norm(v);
  return Boolean(x)&&MARKET_ALIASES[m].some(a=>norm(a)===x);
}
...
function pickPlayer(o:any){
  return String(first(
    o.playerName,o.player_name,o.statEntityName,o.statEntity?.name,o.player?.name,o.player?.displayName,
    o.participantName,o.participant?.name,o.entityName,o.athleteName,o.athlete?.displayName,
    o.description
  )).trim();
}
function pickLine(o:any){
  return safeNumber(first(o.line,o.point,o.total,o.bookOverUnder,o.overUnder,o.bookLine,o.consensusLine,o.value));
}
function pickPrice(o:any){
  return safeNumber(first(o.overPrice,o.over_price,o.price,o.odds,o.bookOdds,o.americanOdds,o.fairOdds));
}
function pickBook(o:any){
  return String(first(o.bookKey,o.book_key,o.bookmaker,o.sportsbook,o.book?.key,o.book?.name,o.provider?.name,o.key)).trim();
}
...
function nextContext(o:any,ctx:Ctx):Ctx{
  ...
  return {
    ...
    book:String(first(o.book?.key,o.book?.name,o.bookmaker?.key,o.bookmaker?.name,o.sportsbook?.key,o.sportsbook?.name,ctx.book)).trim(),
    eventId:String(first(o.eventID,o.eventId,o.event_id,o.gameId,o.id,ctx.eventId)).trim(),
  };
}
function candidateMarket(o:any,m:CfbMarketKey){
  const vals=[o.category,o.market,o.marketKey,o.market_key,o.type,o.stat,o.statName,o.stat_name,o.statID,o.statId,o.betType,o.betTypeID,o.betTypeId,o.label,o.name];
  return vals.some(v=>marketMatch(v,m));
}

export type CfbOwlsRow={
  eventId:string; matchup:string; gameTime:string; playerName:string; teamName:string;
  line:number|null; price:number|null; prob:number|null; bookmakerCount:number;
};
...
export async function getOwlsCfbRows(market:CfbMarketKey):Promise<CfbOwlsRow[]>{
  ...
  const walk=(v:any,ctx:Ctx)=>{
    ...
    seenObjects.add(v);
    const c=nextContext(v,ctx);

    if(candidateMarket(v,market)){
      const playerName=pickPlayer(v);
      const td=market==="anytime_td"||market==="first_td";
      const line=pickLine(v),price=pickPrice(v);
      const side=norm(first(v.side,v.sideID,v.sideId,v.outcome,v.name));
      const wrongSide=!td&&side&&["under","no"].includes(side);
      if(playerName&&!wrongSide&&((td&&(price!=null||line!=null))||(!td&&line!=null))){
        raw.push({
          eventId:c.eventId,matchup:c.matchup,gameTime:c.gameTime,playerName,teamName:c.teamName,
          line,price,prob:americanProb(price),book:c.book||pickBook(v)||"book",
        });
      }
    }
  ...
  for(const row of raw){
    const k=`${cleanName(row.playerName)}|${cleanName(row.matchup)}|${market}`;
    grouped.set(k,[...(grouped.get(k)||[]),row]);
  }
  return [...grouped.values()].map(g=>({
    eventId:g.find(x=>x.eventId)?.eventId||"",
    matchup:g.find(x=>x.matchup)?.matchup||"",
    gameTime:g.find(x=>x.gameTime)?.gameTime||"",
    playerName:g[0].playerName,
    teamName:g.find(x=>x.teamName)?.teamName||"",
    ...
    bookmakerCount:Math.max(new Set(g.map(x=>x.book).filter(Boolean)).size,1),
  }));
}

export async function getOwlsQualifiedGames(markets:CfbMarketKey[]){
  const all=await Promise.all(markets.map(async market=>{
    try{return [market,await getOwlsCfbRows(market)] as const}catch{return [market,[]] as const}
  }));
  const out=new Map<string,{matchup:string;props:CfbMarketKey[]}>();
  for(const [market,rows] of all){
    for(const row of rows){
      if(!row.matchup)continue;
      const k=cleanName(row.matchup);
      const cur=out.get(k)||{matchup:row.matchup,props:[]};
      if(!cur.props.includes(market))cur.props.push(market);
      out.set(k,cur);
    }
  }
  return [...out.values()];
