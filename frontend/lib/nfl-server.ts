import { NFL_MARKETS, type NflMarketKey, type NflRankingRow, type NflRosterPlayer, cleanName, safeNumber } from "@/lib/nfl";

const ESPN_SCOREBOARD="https://site.api.espn.com/apis/site/v2/sports/football/nfl/scoreboard";
const ESPN_STATISTICS="https://site.api.espn.com/apis/site/v2/sports/football/nfl/statistics";
...
const SGO_NORMALIZED:Record<NflMarketKey,Set<string>>=Object.fromEntries(
...
export async function getNflMarketRows(market:NflMarketKey){
...
async function getLeagueStatFallback(market:NflMarketKey){
  try{
...
export async function getNflRankings(market:NflMarketKey):Promise<NflRankingRow[]>{
  const marketRows=await getNflMarketRows(market);
  const out:NflRankingRow[]=[];
  for(const r of marketRows.slice(0,50)){
    ...
    out.push({
      ...
      teamName:profile.teamName||r.matchup.split(" @ ")[0]||"NFL",
      ...
      gamesPlayed:null,
      season:2026,
      summary:`Sportsbook-backed ${NFL_MARKETS.find(x=>x[0]===market)?.[2]||market} ranking. ${r.bookmakerCount||0} book${(r.bookmakerCount||0)===1?"":"s"} contributing.`,
      marketBacked:true,
    });
  }

  if(!out.length){
    const [schedule,leaders]=await Promise.all([getEspnNflSchedule(),getLeagueStatFallback(market)]);
    ...
    for(const r of filtered){
      ...
      const teamName=profile.teamName||r.teamName||"NFL";
      ...
      out.push({
        ...
        gamesPlayed:null,
        season:2026,
        summary:`Verified ESPN 2026 statistical leader fallback for ${NFL_MARKETS.find(x=>x[0]===market)?.[2]||market}. Sportsbook player-prop lines are not currently available, so no betting line or odds were invented.`,
        marketBacked:false,
      });
    }
  }
...
export async function getPropQualifiedGames(){
  const markets:NflMarketKey[]=NFL_MARKETS.map(x=>x[0]);
  const all=await Promise.all(markets.map(async m=>[m,await getNflMarketRows(m)] as const));
  const map=new Map<string,{matchup:string;props:NflMarketKey[]}>();
  for(const [m,rows] of all){
    for(const r of rows){
      const k=cleanName(r.matchup);
      const cur:{matchup:string;props:NflMarketKey[]}=map.get(k)??{matchup:r.matchup,props:[]};
...
export async function getNflTeamRoster(teamId:string):Promise<{teamName:string;teamLogo:string;players:NflRosterPlayer[]}>{
  try{
...
export async function getNflAthleteDetails(athleteId:string){
  let overview:any={};
  let stats:any={};
  ...
  return {
    id:athleteId,
    name:String(athlete.displayName||athlete.fullName||overview.athlete?.displayName||"NFL Player"),
...
export async function getNflGameIntelligence(gameId:string,availableProps:NflMarketKey[]=[]){
  let summary:any={};
  ...
  const leaders:any[]=[];
  const candidateMarkets:NflMarketKey[]=availableProps.length?availableProps:["passing_yards","rushing_yards","receiving_yards"];
