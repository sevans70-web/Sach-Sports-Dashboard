...
export async function saveCfbPregamePredictions(m:CfbMarketKey,rows:any[],schedule:any[]){
 const day=cfbDay(new Date()),existing=await getRow(m,day);if(!existing.connected)return false;
...
export async function getCfbPredictions(m:CfbMarketKey,day:string){
 const x=await getRow(m,day);return {connected:x.connected,predictions:(Array.isArray(x.row?.payload?.predictions)?x.row.payload.predictions:[]) as SavedCfbPrediction[],id:x.row?.id as string|undefined};
}
