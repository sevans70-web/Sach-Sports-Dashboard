...
type Row={
  ...
  giScore:number;prediction:"OVER"|"UNDER"|"FIRST BASKET"|null;summary:string;
  movement?:"new"|"up"|"down"|"same";previousRank?:number|null
};
type RankingResponse={success:boolean;market:NbaMarketKey;rows:Row[];updatedAt?:string;error?:string;source?:string};
...
const meta=(k:NbaMarketKey)=>NBA_MARKETS.find(x=>x[0]===k)!;
...
function predictionText(row:Row,market:NbaMarketKey){
  if(market==="first_basket")return row.prediction==="FIRST BASKET"?"First Basket":"—";
  ...
  const labels:Record<string,string>={
    ...
    rebs_asts:"rebounds + assists",steals:"steals",blocks:"blocks",first_basket:"first basket"
  };
  return `${value} ${labels[market]||meta(market)[2]}`;
}

function Card({row,market}:{row:Row;market:NbaMarketKey}){
  ...
  return <article className="rankCard">
    ...
    <div className="rankPhoto">{row.headshot?<img src={row.headshot} alt=""/>:<div>NBA</div>}{row.teamLogo?<img className="teamLogo" src={row.teamLogo} alt=""/>:null}</div>
    ...
    {open?<div className="detail">
      ...
      <div className="detailMetric"><span>{market==="first_basket"?"MARKET":"REFERENCE LINE"}</span><b>{market==="first_basket"?"First Basket":row.sportsbookLine??"—"}</b></div>
      ...
      <Link className="fullCard" href={`/nba/player/${encodeURIComponent(String(row.playerId))}?market=${market}&name=${encodeURIComponent(row.playerName)}&line=${row.sportsbookLine??""}&projection=${row.modelProjection??""}&gi=${row.giScore}&prob=${row.modelProbability??""}&pick=${row.prediction??""}&team=${encodeURIComponent(row.teamName)}`}>Open full player card</Link>
    </div>:null}
  </article>
}

export function NbaDashboard({data}:{data:NbaOverview}){
  const[market,setMarket]=useState<NbaMarketKey>("points");
  const[performanceMarket,setPerformanceMarket]=useState<NbaMarketKey>("points");
  const[full,setFull]=useState(false);
  const[period,setPeriod]=useState("Today");
  const[movementRows,setMovementRows]=useState<Row[]>([]);
  const[captureTick,setCaptureTick]=useState(0);

  const r=useJson<RankingResponse>(`/api/nba/rankings?market=${market}`,{success:false,market,rows:[]},180000);
  const raw=r.data.rows||[];
  const overallPerf=useJson<PerfResponse>(`/api/nba/performance?period=${period}&capture=${captureTick}`,{connected:false,hits:0,settled:0,pending:0,total:0,hitRate:null,results:[]},60000);
  const marketPerf=useJson<PerfResponse>(`/api/nba/performance?market=${performanceMarket}&period=${period}&capture=${captureTick}`,{connected:false,hits:0,settled:0,pending:0,total:0,hitRate:null,results:[]},60000);

  useEffect(()=>{
    let live=true;
    fetch("/api/nba/capture",{method:"POST",cache:"no-store"})
  ...
  useEffect(()=>{
    ...
    const key=`nba-rank-history-${market}`,eventsKey=`nba-rank-events-${market}`;
  ...
  return <main className="nbaShell">
    ...
    <section className="section performance">
      ...
      <ScrollTabs>{NBA_MARKETS.map(([k,label])=><button className={performanceMarket===k?"active":""} onClick={()=>setPerformanceMarket(k)} key={`perf-${k}`}>{label}</button>)}</ScrollTabs>
    ...
    <section className="section rankings">
      ...
      <ScrollTabs>{NBA_MARKETS.map(([k,label])=><button className={market===k?"active":""} onClick={()=>{setMarket(k);setFull(false)}} key={k}>{label}</button>)}</ScrollTabs>
      <div className="marketHead"><h2>{meta(market)[1]} Rankings</h2><p>GI combines projection strength, the 2025–26 baseline, matchup context and sample reliability. First Basket uses market-implied probability and opening-possession context.</p></div>
    ...
    <style jsx global>{`
      .nbaShell{max-width:780px;margin:0 auto;padding:8px 14px 72px;color:#fff}
      ...
      .heroTop{display:flex;align-items:center;gap:12px}.nbaMenu{display:grid;place-items:center;flex:0 0 44px;width:44px;height:44px;border:2px solid #20df7f;border-radius:13px;background:#0c0e0d;color:#fff!important;text-decoration:none!important;font-size:18px}
