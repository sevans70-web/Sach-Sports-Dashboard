...
export default async function CfbTeamPage({
  ...
  return <main className="teamPage">
    <div className="topRow">
      ...
      <Link href="/cfb/games" className="back">← Back to Games</Link>
    </div>
    ...
    {roster.players.length?<div className="roster">
      {roster.players.map(p=><article className="playerCard" key={p.id}>
        ...
        <Link className="openPlayer" href={`/cfb/player/${encodeURIComponent(p.id)}?name=${encodeURIComponent(p.name)}&team=${encodeURIComponent(name)}&img=${encodeURIComponent(p.headshot||"")}`}>Open Player Card →</Link>
      </article>)}
