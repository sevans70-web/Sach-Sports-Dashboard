...
function marketLabel(key:CfbMarketKey){
  return CFB_MARKETS.find(x=>x[0]===key)?.[2]||"Player Intelligence";
}
...
export default async function CfbPlayerPage({
  ...
  const prob=get("prob")||"—";
  const line=get("line")||"—";
  const requested=(get("market")||"passing_yards") as CfbMarketKey;
  const market=CFB_MARKETS.some(x=>x[0]===requested)?requested:"passing_yards";
  const label=marketLabel(market);

  const teamId=String(details.teamId||"");
  const teamKey=norm(team);
  const currentGame=
    schedule.find((g:any)=>teamId&&(String(g.awayTeamId)===teamId||String(g.homeTeamId)===teamId))||
    schedule.find((g:any)=>teamKey&&(norm(String(g.awayTeam||""))===teamKey||norm(String(g.homeTeam||""))===teamKey));
  ...
  const gameDetail=currentGame
    ?(live||completed?`${currentGame.awayTeam} ${currentGame.awayScore??0} · ${currentGame.homeTeam} ${currentGame.homeScore??0}`:`${currentGame.awayTeam} @ ${currentGame.homeTeam}`)
    :(matchup||"Matchup pending");
  const statusText=currentGame?.status?String(currentGame.status):"Game status pending";
  ...
  return <main className="p">
    ...
    <section className={`gameState ${live?"live":completed?"final":"scheduled"}`}>
      <div><small>GAME STATUS</small><strong>{statusLabel}</strong><span>{statusText}</span></div>
      <div className="gameDetail">{gameDetail}</div>
    </section>

    <section className="strip"><b>{label}</b><span>GI {gi}{prob!=="—"?` · ${prob}%`:""}</span></section>
    ...
    <section className="why">
      ...
      <p>Player intelligence combines available sportsbook information with verified college production and matchup context. Missing sportsbook lines are never invented.</p>
    </section>
    ...
    <CfbPlayerHistory playerId={id} playerName={name} market={market} marketLabel={label} line={line}/>
    ...
    <style>{`
      ...
      .head{display:grid;grid-template-columns:86px 1fr;gap:16px;align-items:center;border:1px solid #34373d;border-radius:18px;padding:18px;background:#111214}.head img,.avatar{width:78px;height:78px;border-radius:50%;border:3px solid #d9b85d;object-fit:cover}.avatar{display:grid;place-items:center;color:#d9b85d;font-weight:900}.head h1{margin:0;font-size:28px}.head p{color:#a9acb3;margin:5px 0}.head b{color:#d9b85d}
      .gameState{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:12px;align-items:center;margin:14px 0 0;border:1.5px solid #34373d;border-radius:13px;padding:12px 14px;background:#0d0f10}.gameState.live{border-color:#20df7f;box-shadow:0 0 0 1px rgba(32,223,127,.16),0 0 18px rgba(32,223,127,.08)}.gameState.final{border-color:#4b4f55}.gameState.scheduled{border-color:rgba(217,184,93,.72)}
      .gameState small{display:block;color:#8f949c;font-size:10px;font-weight:900;letter-spacing:.08em}.gameState strong{display:block;margin-top:3px;color:#d9b85d;font-size:14px}.gameState.live strong{color:#20df7f}.gameState span{display:block;margin-top:3px;color:#a9acb3;font-size:12px}.gameDetail{text-align:right;font-size:12px;font-weight:850;line-height:1.35;color:#fff}
      ...
      .why{border:1px solid #d9b85d;border-radius:14px;padding:16px;margin-top:18px;background:#101112}.why h3{color:#d9b85d;margin-top:0}.why p{line-height:1.5;color:#d7d8db}.rosterButton{display:block;margin-top:14px;border:2px solid #34373d;border-radius:14px;padding:13px;text-align:center;color:#fff;text-decoration:none;background:#0d0f10}
      @media(max-width:600px){.metrics{grid-template-columns:repeat(2,1fr)}.head h1{font-size:23px}.gameState{grid-template-columns:1fr}.gameDetail{text-align:left}}
    `}</style>
  </main>;
}
