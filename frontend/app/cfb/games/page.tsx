...
export default function CfbGames(){
  ...
  useEffect(()=>{
    const run=()=>fetch("/api/cfb/schedule",{cache:"no-store"}).then(r=>r.json()).then(setD).catch(()=>{});
    run();
  ...
  async function toggleGame(x:any){
    ...
    try{
      const payload=await fetch(`/api/cfb/game/${x.id}?props=${encodeURIComponent(props)}`,{cache:"no-store"}).then(r=>r.json());
  ...
  return <main className="gamesPage">
    <MenuButton/>
    <Link href="/cfb" className="backButton">← Back to CFB</Link>
    ...
    {games.length===0?<div className="emptyGames">No CFB games are available in the current slate window.</div>:games.map((x:any)=>{
      ...
      return <section key={x.id} className={`gameCard ${live?"live":""}`}>
        ...
        <div className="teamRow">
          ...
          <div>
            <Link className="teamLink" href={`/cfb/team/${encodeURIComponent(x.awayTeamId)}?name=${encodeURIComponent(x.awayTeam)}&logo=${encodeURIComponent(x.awayLogo||"")}`}>{x.awayTeam}</Link>
        ...
        <div className="teamRow">
          ...
          <div>
            <Link className="teamLink" href={`/cfb/team/${encodeURIComponent(x.homeTeamId)}?name=${encodeURIComponent(x.homeTeam)}&logo=${encodeURIComponent(x.homeLogo||"")}`}>{x.homeTeam}</Link>
        ...
        {open===x.id?<div className="gameIntel">
          ...
          <div className="rosterLinks">
            ...
            <Link href={`/cfb/team/${encodeURIComponent(x.homeTeamId)}?name=${encodeURIComponent(x.homeTeam)}&logo=${encodeURIComponent(x.homeLogo||"")}`}>View {x.homeTeam} roster →</Link>
          </div>
        </div>:null}
      </section>
    })}
