...
export async function GET(req:NextRequest){
 ...
 try{const [board,overview]=await Promise.all([owls(market),loadWnbaOverview()]);const byName=new Map(overview.players.map(p=>[cleanWnbaName(p.playerName),p]));
  const uniqueBoard=[...board.reduce((acc:Map<string,any>,b:any)=>{
   const key=cleanWnbaName(b.playerName);
   const existing=acc.get(key);
   if(!existing){acc.set(key,b);return acc;}
   // Merge duplicate sportsbook/game rows into one player entry per market.
   // Prefer the row backed by more books; otherwise retain the first stable row.
   if(Number(b.bookmakerCount||0)>Number(existing.bookmakerCount||0)) acc.set(key,b);
   return acc;
