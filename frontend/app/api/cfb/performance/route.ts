...
const KEYS:Partial<Record<CfbMarketKey,string[]>>={passing_yards:["passingyards","passyards","yds"],pass_completions:["completions","passingcompletions","cmp"],rushing_yards:["rushingyards","rushyards","yds"],receiving_yards:["receivingyards","receptionyards","recyards","yds"],receptions:["receptions","rec"],anytime_td:["totaltouchdowns","touchdowns","rushingreceivingtouchdowns","td"]};
...
function actualFrom(payload:any,m:CfbMarketKey,day:string){if(!payload||m==="first_td")return null;const names=(Array.isArray(payload.names)?payload.names:[]).map(norm);let ix=-1;for(const k of KEYS[m]||[]){ix=names.findIndex((n:string)=>n===k);if(ix>=0)break}if(ix<0)return null;const events=payload.events||{};for(const st of payload.seasonTypes||[])for(const c of st.categories||[])if(c?.type==="event")for(const ev of c.events||[]){const id=String(ev?.eventId||""),meta=events[id]||{},date=meta?.gameDate||meta?.date||meta?.startDate||ev?.gameDate||ev?.date||"";if(date&&cfbDay(date)!==day)continue;const v=Number(Array.isArray(ev.stats)?ev.stats[ix]:NaN);if(Number.isFinite(v))return v}return null}
...
async function firstTdResult(gameId:string,playerName:string){try{const r=await fetch(`https://site.api.espn.com/apis/site/v2/sports/football/college-football/summary?event=${encodeURIComponent(gameId)}`,{cache:"no-store"});if(!r.ok)return null;const payload=await r.json(),plays=Array.isArray(payload?.scoringPlays)?payload.scoringPlays:[],td=plays.find((x:any)=>{const t=cleanName(String(x?.scoringType?.name||x?.scoringType?.abbreviation||x?.type?.text||"")),text=cleanName(String(x?.text||x?.shortText||""));return t.includes("touchdown")||t==="td"||text.includes("touchdown")});if(!td)return 0;const text=cleanName(String(td?.text||td?.shortText||"")),player=cleanName(playerName),last=player.split(" ").filter(Boolean).pop()||player;return text.includes(player)||(last.length>=3&&text.split(" ").includes(last))?1:0}catch{return null}}
...
export async function GET(req:NextRequest){
  ...
  try{
    ...
    const all=[...batch.predictions];const byBucket=new Map<string,SavedCfbPrediction[]>();
    for(const p of all){const k=`${p.market}|${p.gameDate}`;byBucket.set(k,[...(byBucket.get(k)||[]),p])}
    for(const [bucket,predictions] of byBucket){
      const [market,day]=bucket.split("|") as [CfbMarketKey,string];let changed=false;
      for(const p of predictions){
        if(p.status!=="pending")continue;
        const game=schedule.find((g:any)=>cleanName(`${g.awayTeam} @ ${g.homeTeam}`)===cleanName(p.matchup));if(!game?.completed)continue;
        if(p.market==="first_td"){const actual=await firstTdResult(String(game.id||""),p.playerName);if(actual!=null){p.actual=actual;p.status=settle(p,actual);p.gradedAt=new Date().toISOString();changed=true}continue}
        const payload=await log(p.playerId,Number(p.gameDate.slice(0,4))||2026),actual=actualFrom(payload,p.market,p.gameDate);if(actual!=null){p.actual=actual;p.status=settle(p,actual);p.gradedAt=new Date().toISOString();changed=true}
      }
      if(changed){const db=await getCfbPredictions(market,day);await saveGradedCfbPredictions(market,day,predictions,db.id)}
    }
    ...
    const scoped=marketParam?all:all.filter(p=>groupMarkets.has(p.market)),settled=scoped.filter(p=>p.status==="hit"||p.status==="miss"),hits=scoped.filter(p=>p.status==="hit").length,pending=scoped.filter(p=>p.status==="pending").length;
