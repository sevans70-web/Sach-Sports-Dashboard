...
ESPN is used for fixtures/results and completed-match box scores.  The parser is
intentionally label-driven so one league cannot silently map a stat to the wrong
soccer prop.
"""
...
def load_soccer_scoreboard(league_slug: str, days_back: int = 2, days_forward: int = 7) -> pd.DataFrame:
    now = datetime.now(timezone.utc)
    start = now - timedelta(days=days_back)
    end = now + timedelta(days=days_forward)
    response = requests.get(
        f"{ESPN_BASE}/{league_slug}/scoreboard",
        params={"dates": f"{_date_token(start)}-{_date_token(end)}", "limit": 200},
        timeout=25,
    )
...
def _number(value):
    ...
    if ":" in text:  # minutes are occasionally formatted as 90:00
        text = text.split(":", 1)[0]
    try:
        return float(text)
    except Exception:
...
def _canonical_stat(label: str) -> str | None:
    ...
    aliases = {
        ...
        "sh": "shots", "shots": "shots", "totalshots": "shots",
        "sog": "shots_on_target", "st": "shots_on_target", "shotsontarget": "shots_on_target",
        "g": "goals", "gl": "goals", "goals": "goals",
...
def _athlete_name(athlete: dict) -> str:
    return str(athlete.get("displayName") or athlete.get("shortName") or athlete.get("fullName") or "Unknown")


def _parse_player_groups(summary: dict, game_id: str) -> list[dict]:
    rows: list[dict] = []
    for team_block in summary.get("boxscore", {}).get("players", []) or []:
        ...
        for group in team_block.get("statistics", []) or []:
            ...
            for athlete_row in group.get("athletes", []) or []:
                ...
                parsed = {"minutes": 0.0, "shots": 0.0, "shots_on_target": 0.0, "goals": 0.0, "assists": 0.0, "saves": 0.0}
                found = False
...
@st.cache_data(ttl=1800, show_spinner=False)
def load_recent_player_stats(league_slug: str, completed_game_ids: tuple[str, ...]) -> pd.DataFrame:
    rows: list[dict] = []
    # Cap protects Streamlit/API performance while still giving the engine a recent sample.
    for game_id in completed_game_ids[-20:]:
        try:
            rows.extend(_parse_player_groups(load_match_summary(league_slug, game_id), game_id))
        except Exception:
            continue
    return pd.DataFrame(rows)


def recent_stats_for_scoreboard(league_slug: str, scoreboard: pd.DataFrame) -> pd.DataFrame:
    if scoreboard.empty or "completed" not in scoreboard:
        return pd.DataFrame()
    completed = scoreboard[scoreboard["completed"]].sort_values("kickoff")
    ids = tuple(completed["game_id"].dropna().astype(str).tolist())
