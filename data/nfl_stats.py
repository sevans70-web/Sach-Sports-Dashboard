...
def load_nfl_weekly_player_stats(season=2025):
    ...
    for c in ["week","completions","attempts","passing_yards","passing_tds","interceptions","carries","rushing_yards","rushing_tds","targets","receptions","receiving_yards","receiving_tds"]:
        if c in df.columns:
...
def load_nfl_season_baseline(season=2025):
    ...
    ids = [c for c in ["player_id","player_display_name","position","recent_team"] if c in weekly.columns]
    ...
    base = weekly.groupby(ids, dropna=False)[stats].sum().reset_index()
    games = weekly.groupby(ids, dropna=False)["week"].nunique().reset_index(name="games_played")
    base = base.merge(games, on=ids, how="left")
