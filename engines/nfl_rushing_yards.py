...
def _baseline_projection(row):
    return season_anchored_projection(
        row.get("rushing_yards_per_game"),
        row.get("last_5_rushing_yards_per_game"),
        row.get("last_3_rushing_yards_per_game"),
        max_adjustment=4.0,
        digits=1,
    )
...
def attach_rushing_market(df: pd.DataFrame) -> pd.DataFrame:
    ...
    result["rushing_projection_unanchored"] = pd.to_numeric(
        result["rushing_baseline_projection"], errors="coerce"
    )
    result["rushing_projection"] = market_anchored_projection(
        result["rushing_projection_unanchored"],
