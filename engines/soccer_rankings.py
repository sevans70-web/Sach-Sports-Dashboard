...
def build_soccer_rankings(
    ...
    """Create rankings from real posted markets, enriched by recent stats."""
    ...
    if (
        ...
        working = stats.copy()
        working["name_key"] = (
            working["player_name"].map(_norm_name)
        )
        ...
        if "photo_url" in working.columns:
            agg_spec["photo_url"] = (
                "photo_url",
                lambda s: next(
                    (
    ...
    market["name_key"] = (
        market["player_name"].map(_norm_name)
    )
    ...
    if "photo_url" not in market.columns:
        market["photo_url"] = ""

    market["history_available"] = (
        market["metric_avg"].notna()
    )
    ...
    market["history_adjustment"] = market.apply(
        history_adjustment,
        axis=1,
    )
    ...
    market["model_probability"] = (
        market["market_probability"].round(1)
    )
    ...
    def why(row):
        ...
        return (
            f"{base} • market-led until "
            f"recent match history is available"
        )

    market["why_engine"] = market.apply(
        why,
        axis=1,
    )
    ...
    return market[
        [c for c in keep if c in market.columns]
    ]
