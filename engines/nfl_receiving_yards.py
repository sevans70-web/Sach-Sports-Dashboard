...
def _baseline_projection(row):
    season_avg = row.get("receiving_yards_per_game")
    last_5 = row.get("last_5_receiving_yards_per_game")
    last_3 = row.get("last_3_receiving_yards_per_game")
...
def attach_receiving_market(df: pd.DataFrame) -> pd.DataFrame:
    ...
    result["receiving_projection"] = pd.to_numeric(
        result["receiving_baseline_projection"],
        errors="coerce",
    )
