"""NFL Receiving Yards projection, market, probability and Top 25 ranking."""
...
from data.nfl_odds import load_nfl_receiving_yards_markets
...
DEFAULT_RECEIVING_VOLATILITY = 24.0
...
def build_receiving_yards_foundation(
    roster_season: int = ROSTER_SEASON,
    baseline_season: int = BASELINE_SEASON,
) -> pd.DataFrame:
    """Build current-roster receiving baselines from the prior season."""
    ...
    weekly["receiving_yards"] = pd.to_numeric(
        weekly["receiving_yards"],
        errors="coerce",
    ).fillna(0)
    ...
    for player_id, group in weekly.groupby("player_id"):
        ...
        recent_rows.append(
            {
                "player_id": player_id,
                "last_5_receiving_yards_per_game": (
                    last_5["receiving_yards"].mean()
                    if not last_5.empty
                    else pd.NA
                ),
                "last_3_receiving_yards_per_game": (
                    last_3["receiving_yards"].mean()
                    if not last_3.empty
                    else pd.NA
                ),
    ...
    players["receiving_yards_per_game"] = pd.to_numeric(
        players.get("receiving_yards_per_game"),
        errors="coerce",
    )

    players["receiving_baseline_projection"] = players.apply(
        _baseline_projection,
        axis=1,
    )

    players["receiving_data_status"] = players.apply(
        _data_status,
        axis=1,
    )
...
def _baseline_projection(row):
    return season_anchored_projection(
        row.get("receiving_yards_per_game"),
        row.get("last_5_receiving_yards_per_game"),
        row.get("last_3_receiving_yards_per_game"),
        max_adjustment=4.0,
        digits=1,
    )
...
def _prepare_market():
    markets = load_nfl_receiving_yards_markets().copy()
...
def attach_receiving_market(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty:
        return df.copy()
    ...
    result["receiving_projection"] = pd.to_numeric(
        result.get("receiving_baseline_projection"),
        errors="coerce",
    )
    ...
    result["receiving_projection_unanchored"] = pd.to_numeric(
        result["receiving_baseline_projection"], errors="coerce"
    )
    result["receiving_projection"] = market_anchored_projection(
        result["receiving_projection_unanchored"],
        result["consensus_line"],
        maximum_distance=6.0,
    ).round(1)

    result["projection_edge_yards"] = (
        result["receiving_projection"]
        - pd.to_numeric(
            result["consensus_line"],
            errors="coerce",
        )
...
def attach_receiving_probabilities(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty:
        return df.copy()
...
def rank_receiving_yards_top25(
    df: pd.DataFrame,
    limit: int = 25,
) -> pd.DataFrame:
    ...
    established = (ranked["receiving_data_status"] == "Established baseline")
    valid_projection = ranked["receiving_projection"].notna() & (ranked["receiving_projection"] > 0)
    live_mask = (
    ...
    ranked = (
        ranked.sort_values(
            ["model_probability", "abs_model_edge", "receiving_projection"],
...
def build_receiving_yards_top25(
    roster_season: int = ROSTER_SEASON,
    baseline_season: int = BASELINE_SEASON,
) -> pd.DataFrame:
    foundation = build_receiving_yards_foundation(
        roster_season,
        baseline_season,
    )
    foundation = attach_receiving_market(
        foundation
    )
    foundation = attach_receiving_probabilities(
        foundation
    )
    return rank_receiving_yards_top25(
        foundation,
        limit=25,
    )
