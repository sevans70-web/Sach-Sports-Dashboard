"""NBA schedule and scoreboard data helpers for Sach Sports Dashboard."""
...
def load_nba_scoreboard(start_date: str, end_date: str) -> pd.DataFrame:
    """Load NBA games for an inclusive YYYY-MM-DD date range from ESPN."""
...
def current_nba_window(days_back: int = 1, days_forward: int = 14) -> pd.DataFrame:
    """Return a practical current/upcoming NBA window in Toronto local time."""
    today = datetime.now(TORONTO_TZ).date()
    return load_nba_scoreboard(
