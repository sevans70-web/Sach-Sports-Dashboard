"""WNBA current-season baseline ranking engine.

These rankings are statistical baselines from the active 2026 regular season.
...
from data.wnba_stats import WNBA_BASELINE_SEASON, load_wnba_player_baseline
...
def build_wnba_baseline_top25(
    prop: str,
    season: str = WNBA_BASELINE_SEASON,
    minimum_games: int = 10,
) -> pd.DataFrame:
    """Build a real-data Top 25 baseline for one supported WNBA prop."""
    if prop not in PROP_METRICS:
        raise ValueError(f"Unsupported WNBA prop: {prop}")

    stats = load_wnba_player_baseline(season).copy()
    if stats.empty:
        return stats
