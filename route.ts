...
export async function GET(req:NextRequest){
 ...
 try{const [board,overview]=await Promise.all([owls(market),loadWnbaOverview()]);const byName=new Map(overview.players.map(p=>[cleanWnbaName(p.playerName),p]));
  const rows=board.map((b:any)=>{const p=byName.get(cleanWnbaName(b.playerName));const baseline=playerBaseline(p,market);const edge=baseline!=null&&b.line!=null?baseline-b.line:null;const reliability=Math.min(1,Math.max(0,(p?.gamesPlayed??0)/30));const edgePct=edge!=null&&b.line?Math.min(1,Math.abs(edge)/Math.max(1,Math.abs(b.line))):0;const gi=Math.round((50+edgePct*35+reliability*15)*10)/10;const modelProbability=edge==null?null:Math.max(50,Math.min(82,Math.round((54+Math.abs(edge)*2.2+reliability*5)*10)/10));return{playerId:p?.playerId||cleanWnbaName(b.playerName),playerName:b.playerName,teamName:p?.team||"WNBA",matchup:b.matchup,gameTime:b.gameTime,headshot:p?wnbaHeadshot(p.playerId):"",sportsbookLine:b.line,bookmakerCount:b.bookmakerCount,modelProjection:baseline,modelProbability,giScore:gi,prediction:edge==null?null:edge>=0?"OVER":"UNDER",summary:baseline==null?`Verified sportsbook line ${b.line}. Statistical baseline is still loading.`:`2026 baseline ${baseline.toFixed(1)} vs verified line ${Number(b.line).toFixed(1)} (${edge>=0?"+":""}${edge.toFixed(1)} edge). ${b.bookmakerCount} sportsbook source${b.bookmakerCount===1?"":"s"} currently represented.`}});
